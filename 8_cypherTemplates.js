...
function buildCypher(plan) {
  ...
  steps.forEach(step => {
    if (step.type === "filter") requiredLabels.add(step.field.split(".")[0]);
    if (step.type === "projection") step.fields.forEach(f => requiredLabels.add(f.split(".")[0]));
    if (step.type === "sort") requiredLabels.add(step.field.split(".")[0]);
  });

  requiredLabels.forEach(label => {
    const varName = LABEL_VAR_MAP[label];
    const alreadyMatched = steps.some(s => s.type === "traversal" && (s.from === label || s.to === label));
    if (!alreadyMatched) {
      matchClauses.push(`MATCH (${varName}:${label})`); // Now this works!
    }
  }); 
  // fixed
  const whereClauses = [];
